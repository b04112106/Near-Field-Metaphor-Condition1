...
public class PersonalSpace : MonoBehaviour
{
    ...
    private GameObject bound, menu, GM;
    ...
    private void CreateCopy()
    {
        ...
        CopyOfObject = Instantiate(this.gameObject, Cam.transform.position + Cam.transform.forward * distance - new Vector3(0,0.15f,0), this.gameObject.transform.rotation);
        ...
        CopyOfObject.GetComponent<MeshRenderer>().enabled = false;
        CopyOfObject.GetComponent<BoxCollider>().enabled = false;
    }
    ...
    private void CreateBoundingBox()
    {
        ...
        bound.transform.localScale *= 1.25f;
        ...
        bound.transform.Translate(0,-1000f,0);
        // // adjust edge size
        // for(int i=0; i<4; i++)
    ...
    public void clean()
    {
        ...
        if (GameObject.FindWithTag("CopyOfObject"))
            Destroy(GameObject.FindWithTag("CopyOfObject"));
        if(contextInPersonal.Count != 0)
        {
            for(int i=0; i<contextInPersonal.Count; i++)
                Destroy(contextInPersonal[i]);
